...
def test_config_can_disable_shared_store_instance(monkeypatch):
    module = __import__(
        "config.encapsulation.database.graph_db.pruned_hipporag_neo4j_config",
        fromlist=["PrunedHippoRAGNeo4jStore"],
    )
    cached_factory = _make_cached_store_factory()
    monkeypatch.setattr(module, "PrunedHippoRAGNeo4jStore", cached_factory)

    cfg_shared = PrunedHippoRAGNeo4jConfig(
