...
async def test_deepsearch_run_async_exposes_progress_and_result():
    class _StubAccount:
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test", timeout=None) as client:
        ...
        progress = await asyncio.wait_for(client.get(f"/deepsearch/progress/{run_id}"), timeout=5.0)
        assert progress.status_code == 200

        deadline = time.time() + 3.0
        while time.time() < deadline:
            result = await asyncio.wait_for(client.get(f"/deepsearch/result/{run_id}"), timeout=5.0)
...
async def test_deepsearch_stream_emits_done_marker():
    class _StubAccount:
        def get_user_by_username(self, username: str):
            return None
...
async def test_deepsearch_stream_openai_format_emits_tool_calls():
    class _StubAccount:
        def get_user_by_username(self, username: str):
            return None
