...
class DeepSearchReporter:
    ...
    async def compose(
        ...
        evidences = self._attach_provenance_chunk_evidence(trace, evidences, question=question)
    ...
    def _attach_provenance_chunk_evidence(
        self,
        trace: Dict[str, Any],
        evidences: List[Dict[str, Any]],
        *,
        ...
        if not evidences or self.graph_store is None:
            return evidences
        execute = getattr(self.graph_store, "_execute_query", None)
        if not callable(execute):
            return evidences

        max_attach = int(self.provenance_chunk_attach_max or 0)
        if max_attach <= 0:
            return evidences
        ...
        owner_key = scope_id
        owner_key_fn = getattr(self.graph_store, "_owner_key", None)
        try:
            if callable(owner_key_fn):
                owner_key = str(owner_key_fn(scope_id))
        except Exception:
            owner_key = scope_id
        global_owner = getattr(self.graph_store, "OWNER_GLOBAL_KEY", "__GLOBAL__")

        # Avoid duplicates when a chunk is already present as authoritative evidence.
        ...
        for item in evidences:
            ...
            if chunk_id and content and not chunk_id.startswith(composer_defaults.TOOLISH_CHUNK_ID_PREFIXES):
        ...
        # Collect chunk ids referenced by deterministic tool evidence provenance.
