...
def _render_tool_call(payload: Dict[str, Any]) -> str:
    ...
    if tool_name == "code.python":
        ...
        if isinstance(code, str) and code.strip():
            ...
            lines.append("code_preview=" + _truncate_text(preview, limit=0))
    ...
    if tool_name == "code.python" and isinstance(result_obj, dict):
        ...
        if env:
            lines.append(
                "env="
                + json.dumps(
                    {"python_version": env.get("python_version"), "platform": env.get("platform")},
                    ensure_ascii=False,
                    separators=(",", ":"),
                    default=str,
                )
        ...
        if isinstance(err, dict) and (err.get("message") or err.get("type")):
            ...
            lines.append("exec_error=" + _truncate_text(message, limit=0))
        ...
        for raw in evidences[:sample_n]:
            ...
            if isinstance(triples, list):
                for triple in triples:
                    if isinstance(triple, dict) and triple.get("head") and triple.get("relation") and triple.get("tail"):
...
def format_weaver_progress(event_type: str, payload: Any, *, run_id: str) -> str:
    ...
    if percent is not None:
        try:
            lines.append(f"percent={int(percent)}")
        except Exception:
            pass
    message = payload.get("message")
    if isinstance(message, str) and message.strip():
        lines.append("message=" + _truncate_text(message.strip(), limit=0))
    if stage_record:
        lines.append("stage_record=" + json.dumps(stage_record, ensure_ascii=False, separators=(",", ":"), default=str))
    if stage_history:
        lines.append("stage_history=" + json.dumps(stage_history, ensure_ascii=False, separators=(",", ":"), default=str))
    if errors:
