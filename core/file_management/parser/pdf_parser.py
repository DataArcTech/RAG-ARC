...
class DotsOCRParser:
    ...
    def _load_hf_model(self):
        ...
        model_path = script_dir / "weights" / "DotsOCR"
        if not model_path.exists():
            print('Try to download model to model_path')
            try:
                from huggingface_hub import snapshot_download
                snapshot_download(repo_id="rednote-hilab/dots.ocr", local_dir=model_path, local_dir_use_symlinks=False, resume_download=True)
            except:
                from modelscope import snapshot_download
                snapshot_download(repo_id="rednote-hilab/dots.ocr", local_dir=model_path)
    ...
    def parse_file(self, 
        ...
        self.process_output(filename=filename, results=results, output_dir=output_dir, save_layout=save_layout)

        new_path = Path(os.path.join(output_dir, os.path.basename(filename)+'_original.pdf'))
        new_path.write_bytes(Path(input_path).read_bytes()) 


        return results
    
    def read_json(self, path, page_no, h, w):
    ...
    def process_output(self, filename, results, output_dir, save_layout=True):
        layout = []
        output_md = os.path.join(output_dir, filename+"_markdown.md")
        output_json = os.path.join(output_dir, filename+'_layout.json')
        with open(output_md, 'w', encoding='utf-8') as output_f:
            for item in tqdm(results):
                # item = json.loads(line)
                page_no = item.get("page_no",0)
                layout_file = item.get('layout_info_path','')
                md_file = item.get("md_content_path",'')

                h = item.get('input_height',0)
                w = item.get('input_width',0)

                output_f.write(self.read_md(md_file))
                output_f.write("\n\n")
                try:
                    layout = layout + self.read_json(layout_file, page_no, h, w)
                except:
                    layout = layout
        if save_layout:
...
def main():
    ...
    parser.add_argument(
        "--save_layout", type=str, default=True,
        help="Save layout file"
    )
    ...
    if os.path.isdir(args.input_path):
        pdf_suffixes = [".pdf"]
        for doc_path in sorted(Path(args.input_path).glob('*')):
        ...
        print(f"'{args.input_path}' no exist")
