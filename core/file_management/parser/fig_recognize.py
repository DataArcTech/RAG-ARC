...
def save_fig(file_path, page_no, index, bbox, scale):
    """
    args:
      file_path: layout json文件路径
      page_no: 图像所在页码数
      index: 图像所在页码中具体的文本块
      bbox: 图像的x1,y1,x2,y2
      scale: bbox对应pdf的长,宽(dots 的bbox经过放大)
    return:
      save_path: 保存路径
      text: 图像识别内容
    """
    ...
    try:
        ...
        print("Please save original pdf file with layout file.")
        return None, None
...
def process_one_file(json_file, no_replace):
    file_name = os.path.basename(json_file)
    base_dir = os.path.dirname(json_file)
    output_path = str(json_file).replace("layout", "img_content")
    pdf_file = str(json_file).replace("layout.json", "original.pdf")
    if not os.path.exists(pdf_file):
        print("Please save original pdf file with layout file.")
        return None

    if os.path.exists(output_path) and no_replace:
        print(f"{output_path} has already exited.")
        return None
...
def main():
    ...
    parser.add_argument(
        "--no_replace", type=bool, default=True,
        help="Whether replece the exited file"
    )
    args = parser.parse_args()


    if os.path.isdir(args.output):
        files = sorted(Path(args.output).glob('*_layout.json'))
        if len(files) == 0:
            print("no layout json file")
        else:
            for file in files:
                data = process_one_file(file, args.no_replace)
    elif os.path.isfile(args.output):
        data = process_one_file(args.output, args.no_replace)
    else:
