...
class DeepSearchService:
    ...
    async def _classify_question(self, *, question: str, routing_cfg: Dict[str, Any]) -> Dict[str, Any]:
        ...
        llm = getattr(self.planner, "llm_connector", None)
        ...
        if routing_cfg.get("_llm_error"):
            payload["llm_error"] = routing_cfg.get("_llm_error")
        return payload

    @staticmethod
    def _count_deterministic_tool_runs(reasoning_trace: Dict[str, Any]) -> int:
        tool_runs = reasoning_trace.get("tool_results") or []
    ...
    def _maybe_hard_gate_computable_question(
        ...
        *,
        ...
        structured_report = {
            "format_version": str(getattr(self.reporter, "STRUCTURED_REPORT_VERSION", "2.0")),
            "title": question,
            "short_answer": msg,
            "summary": msg,
            "sections": [],
