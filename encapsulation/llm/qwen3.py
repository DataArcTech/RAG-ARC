...
class QwenConfig(LLMBaseConfig):
    """
    ...
    def build(self) -> "QwenLLM":
        """Build the Qwen reranker model"""
        return QwenLLM(config=self)


@dataclass
class QwenLLM(LLMBase[QwenConfig]):
    """
    ...
    def _init_model(self):
        ...
        try:
            ...
            if 'torch_dtype' in model_kwargs:
                dtype_str = model_kwargs.pop('torch_dtype')
            ...
            self._client.to(self.config.device)
            ...
            self.logger.info(f"Qwen LLM (Reranker) initialized: {self.config.model_name}")
    ...
    def _process_inputs(self, pairs):
        ...
        # Move to device
        for key in out:
            out[key] = out[key].to(self.config.device)
