...
class _PrunedHippoRAGIGraphIndexingMixin:
        ...
        try:
            with self.write_lock():
                ...
                for i, chunk in enumerate(chunks):
                    ...
                    if chunk.graph and not chunk.graph.is_empty():
                        logger.info(
                            "    Adding graph data: %d entities, %d relations",
                            len(chunk.graph.entities),
                            len(chunk.graph.relations),
                        )
                        self.add_graph_data(chunk.graph, chunk.id, owner_id=chunk.owner_id)
                    else:
                        logger.warning("    Chunk %s has no graph data", chunk.id)
                ...
                # Step 4: Rebuild graph from SQLite so PPR traversal sees the new nodes/edges.
                logger.info("Step 4: Rebuilding graph...")
                self.build_graph()
                logger.info("Step 4 completed: Graph rebuilt")

                # Step 5: Rebuild chunk embeddings array
                logger.info("Step 5: Rebuilding chunk embeddings array...")
                self._rebuild_chunk_embeddings_array()
                logger.info("Step 5 completed: Chunk embeddings array rebuilt")

                # Step 6: Final commit to ensure all changes are persisted
                logger.info("Step 6: Committing all changes to database...")
                self.conn.commit()
                logger.info("Step 6 completed: All changes committed")
