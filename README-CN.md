...
RAG-ARC 支持通过 `.env` 切换 PDF/图片解析方式：

- `PARSER_PARSE_MODE=native`（默认）：不做 OCR；仅对 PDF 做文本抽取（图片不支持）。
...
- `locate`：文件路由；带 `file=<file_id>` 时返回该文件内的页级候选
- `toc.tree`：目录导航（标题 + 页码范围）
- `read.pages`：读取目标页的完整上下文（唯一可引用证据来源）

对于可计算问题（`query_spec.is_computable=true`），DeepSearch 在停止前必须执行一次确定性计算工具 `code.python`。
...
4. **（可选）Dense 引导的文件先验**：当 dense top-K Chunk 明显集中在单一文件时，提高该文件的 passage reset 权重，减少“相似产品串召回”，同时仍保持 PPR 排序
