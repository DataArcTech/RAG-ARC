...
| `PAGEINDEX_STRICT_PAGE_CHUNKING_MODE` | `auto` | Strict page-aware chunking: `off` disables; `auto` uses MinerU content_list when available (fallback otherwise); `require` fails indexing if page text is missing. |
...
| `LLM_JSON_RETRY_DEFAULT_ATTEMPTS` | `2` | Global default attempts for LLM JSON contracts (including the first generation). Used by DeepSearch and non-DeepSearch JSON parsing flows unless overridden by module-specific knobs. |
| `LLM_JSON_RETRY_MAX_ATTEMPTS` | `8` | Global upper bound for JSON retry loops to avoid runaway retries. |
| `LLM_JSON_RETRY_DEFAULT_TEMPERATURE` | `0.0` | Retry temperature used when repairing malformed JSON outputs. |
| `LLM_JSON_RETRY_DEFAULT_MAX_RAW_CHARS` | `2000` | Max characters of the previous invalid output injected back into repair prompts. |
| `RAG_RETRIEVAL_WEIGHT_DENSE` | `1.0` | MultiPath RRF fusion weight for the dense retriever. Set to `0` to disable this retrieval path in normal RAG. |
